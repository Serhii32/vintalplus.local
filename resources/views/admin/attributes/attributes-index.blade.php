                        ...
                        @if(count($attributesNames))
                            <div class="container-fluid">
                                ...
                                @foreach($attributesNames as $attributesName)
                                    <div class="row border">
                                        <div class="col-md-4 py-2 border-right">
                                            <h4><a class="card-link text-dark" href="{{route('admin.attributes.edit', [$attributesName->id, 'name=1'])}}">{{$attributesName->nameRU}}</a></h4>
                                        </div>
                                        <div class="col-md-8 py-2">
                                            @foreach($attributesName->values()->get() as $attributesValue)
                                                <h4 class="text-dark"><a class="card-link text-dark" href="{{route('admin.attributes.edit', [$attributesValue->id, 'value=1'])}}">{{$attributesValue->valueRU}}</a></h4>
                                            @endforeach
                                        </div>
                                    </div>
                                @endforeach
                                @foreach($attributesValues as $attributesValue)
                                    <div class="row border">
                                        <div class="col-md-4 py-2 border-right"></div>
                                        <div class="col-md-8 py-2">
                                            <h4 class="text-dark"><a class="card-link text-dark" href="{{route('admin.attributes.edit', [$attributesValue->id, 'value=1'])}}">{{$attributesValue->valueRU}}</a></h4>
                                        </div>
                                    </div>
